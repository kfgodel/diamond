  ...
  public void define() {
    describe("a type variable representation", () -> {
      ...
      it("can have superclass from upper bounds", () -> {
        assertThat(context().typeInstance().runtime().hierarchy().superclass().map(Named::name).get())
          .isEqualTo("Number");
      });

      it("can have interfaces from upper bounds", () -> {
        List<String> interfaceNames = context().typeInstance()
          .runtime().hierarchy().interfaces()
          .map(Named::name)

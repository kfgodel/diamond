    ...
    public void define() {
        describe("a generator iteration", ()->{
            ...
            it("starts with a supplied value",()->{
                GeneratorSpliterator<Integer> integerSpliterator = GeneratorSpliterator.create(Optional.of(1));
            ...
            it("uses a function to get the next item",()->{
                GeneratorSpliterator<Integer> infiniteSpliterator = GeneratorSpliterator.create(Optional.of(1), (integer) -> Optional.of(integer + 1));
            ...
            it("iteration finishes when last value generator returns an empty optional",()->{
                GeneratorSpliterator<Integer> integerSpliterator = GeneratorSpliterator.create(Optional.of(1), (integer) -> (integer == 4)? Optional.empty(): Optional.of(integer + 1));
                assertThat(integerSpliterator.toStream().collect(Collectors.toList()))
                        .isEqualTo(Arrays.asList(1,2,3, 4));
            });
            
            it("has characteristics supplied to describe it",()->{
                GeneratorSpliterator<Integer> spliterator = GeneratorSpliterator.create(Optional.of(1), (integer) -> Optional.of(integer + 1), Spliterator.NONNULL | Spliterator.DISTINCT | Spliterator.CONCURRENT);
            ...
            it("can be used to iterate an empty set",()->{
                GeneratorSpliterator<Integer> integerSpliterator = GeneratorSpliterator.create(Optional.empty());
                assertThat(integerSpliterator.toStream().collect(Collectors.toList()))
